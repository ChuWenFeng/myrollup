...
export default {
    ...
    methods: {
        async deposit() {
            ...
            let value = this.depositAmount
            let from = store.account.address
            value = Eth.toWei(value, 'ether').div(new BN('1000000000000')).toNumber();
            //let hash = await contract.deposit([pub.x, pub.y], maxFee, { value, from })
            //let privatekey = store.account.plasma.key.privateKey
            
            let apiForm = {
                address: from,
                public_key: [pub.x, pub.y],
                deposit_amount: value
            }

            let result = await axios({
                method:'post',
                url:this.baseUrl + "/depositreq",
                data:apiForm
            })

            this.alert(result.data, 'success')
        },
        async withdrawSome() {
            try {
        ...
        async withdrawAll() {
            ...
            let result = await axios({
                method: 'get',
                url: this.baseUrl + "/exit/" + from,
            })

            //this.alert('Full exit initiated, tx: ' + hash, 'success')
            if (result.data.error != null){
                this.alter(result.data.error)
            }else{ 
                this.alert(result.data,sucess)
            }
        },
        ...
        async transfer() {
            ...
            this.transferPending = true
                ...
                let result = await axios({
                    method: 'get',
                    url:    this.baseUrl + '/address/' + this.transferTo,
                })

                let to = result.data.account_id
        ...
        async updateAccountInfo() {
            ...
            try {
                ...
                let result = await axios({
                    method: 'get',
                    url:    this.baseUrl + '/address/' + store.account.address,
                })
                let id = result.data.account_id
